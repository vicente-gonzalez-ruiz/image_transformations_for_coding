...
\tightlist
\item
  Divide \(x=\{x_n\}_{n=0}^{N-1}\) into \(\lceil N/B \rceil\) blocks
  \(\{x_1, \cdots, x_{\lceil N/B \rceil}\}\) of \(B\) samples.
\end{enumerate}

\section{Transform of a block $x=\{x_n\}_{n=0}^{N-1}$}
\begin{itemize}
\item
  In the forward transform the samples of the block $x$ are
  projected (notice that this operation is a
  \href{https://en.wikipedia.org/wiki/Dot_product}{dot product}) on a
  set of basis functions
  \begin{equation}
    y_n = \sum_{i=0}^{N-1}a_{n,i}x_i,
    \tag{forward\_transform}
  \end{equation}
  where $\{y_n\}$ is the transformed sequence and $\{x_n\}$ is the
  original sequence.
\item
  The backward (inverse) transform restores the original samples
  \begin{equation}
    x_n = \sum_{i=0}^{N-1}b_{n,i}y_u.
...
\item
  ...
  \begin{equation}
    y = Ax = \begin{bmatrix}
      a_{0,0} & \cdots & a_{{B-1},0} \\
      \vdots & & \vdots \\
      a_{0, B-1} & cdots & a_{B-1, B-1}
    \end{bmatrix}
    \begin{bmatrix}
      x_0 \\
      \vdots \\
  ...
  \begin{equation}
    \begin{array}{l}
    [A]_{i,j} = a_{i,j} \\\relax
    [B]_{i,j} = b_{i,j}.
    \end{array}
  \end{equation}

  $[A]_i$ is called a basis vector of the transformation matrix $A$. If
  the transform is orthonormal, they form an orthonormal basis set.
...
Therefore, the sum of the squares of the transformed sequence is the
...
\item
  ...
  \begin{equation}
    G=\frac{\frac{1}{N}\displaystyle\sum_{i=0}^{N-1}\sigma_{y_i}^2}{\sqrt[N]{\displaystyle\prod_{i=0}^{N-1}\sigma_{y_i}^2}},
  \end{equation}

  where \(\sigma_{y_i}^2\) is the variance of coefficient \(y_i\).
\end{itemize}
